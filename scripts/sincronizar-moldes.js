#!/usr/bin/env node
/**
 * Sincroniza los tickets de Moldes desde osTicket hacia la réplica local.
 *
 * El servidor lo corre solo cada 15 minutos; este script sirve para la carga
 * inicial y para forzar una corrida a mano.
 *
 *   node scripts/sincronizar-moldes.js             incremental
 *   node scripts/sincronizar-moldes.js --completa  trae todo el histórico
 *   node scripts/sincronizar-moldes.js --estado    solo informa, no sincroniza
 */
require("dotenv").config();
const { sincronizar, estado } = require("../src/services/sincronizarMoldes.service");
...
async function main() {
  ...
  if (args.includes("--estado")) {
    ...
    console.log(`Réplica: ${n(e.tickets)} tickets`);
